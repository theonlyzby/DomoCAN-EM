...
function Temps() {
  ...
  if (($action=="update") OR ($action=="AddTemp")) {
    ...
    while (isset($IdArray[$i])) { 
      $sql = mysql_real_escape_string("UPDATE `chauffage_sonde` SET `img_x` = \"" . 
             $YcoorArray[$i] . "\", " . "`img_y` = \"" .
    ...
    if ($action=="AddTemp") {
      ...
      $sql = mysql_real_escape_string("INSERT INTO `chauffage_sonde` (`id`, `id_sonde`, `moyenne`, " .
			 "`valeur`, `localisation`, `img_x`, `img_y`, `description`, `update`) VALUES " .
			 "(NULL, \"" . $id_sonde . "\", \"" . $Moyenne . "\", \"00.00\",  " .
			 "\"" . $selected_level . "\", \"1\", \"1\", \"" . $Temp_Name . "\", NOW() );");
	  $sql = str_replace(chr(92).chr(34),"'",$sql);
  ...
  if ($action=="AddLevel") {
	...
	if (($Level_Name!="") AND ($File_Name)) {
	  ...
	  if(copy($_FILES['NewPlan']['tmp_name'], $Dest_Name)) {
	    ...
	    if (!png2jpg($Dest_Name, $Dest_Jpeg, 100)) { log_this("Probleme de copie de Fichier .JPG"); } // End IF
      } else {
    ...
    if ($action=="DeleteLevel") {
	  ...
	  if ((@unlink("../www/images/plans/plan_" . $Level . ".png")) AND (@unlink("../www/images/plans/plan_" . $Level . ".jpg"))) {
	    ...
	    log_this("Impossible d'effacer les Fichiers");
      }	  // End IF
    } // End IF
  ...
  echo("<h2 class='title'>" . ADMIN_TEMP_PAGE_NAME . "</h2>");
...
document.getElementById("moveMe"+e).src="../www/images/fond_temperature_silver.png";
  ...
  $sql = mysql_real_escape_string("SELECT * FROM `chauffage_sonde` WHERE " .
...
<?php
  ...
  echo("<div id='text-11' class='block widget_text'><br><br><br><h2>" . ADMIN_TEMP_SIDE_TITLE . "</h2>" . CRLF);
  ...
  while (isset($id[$i])) {
    echo("  <li><input type='text'" .
         ...
         " value='" . $description[$i] . 
         "' />" . CRLF);
	echo("<a href='javascript:void();' onClick='TempDelete(".$id[$i].");'><img src='./images/drop.png'/></a></li>" . CRLF);
...
&nbsp;&nbsp;<a href='javascript:void(1);' onClick='showOverlay("NewTemp","Temp_Name");'><img width='14' height='14' align='absmiddle' src='../www/images/ajouter.png' />&nbsp;&nbsp;&nbsp;Ajouter une Sonde</a>
  </ul></div>
...
function CheckDropSubmitform(field,drop,action) {
  ...
  if (ll.length>=1) {
    if (dd.length>=1) {
      ...
      alert("Pas de Sonde S\351lectionn\351e!");
	  return;
    }
  } else {
...
<a href="javascript:void(1);" onClick="hideOverlay('NewTemp');"><img width='14' height='14' align='absmiddle' src='../www/images/close.jpg' />&nbsp;Fermer</a>

<p align=center><h1 align=center>Ajouter une Sonde au Niveau <?php echo($selected_level);?></h1></p>
<br><br><br><br>
<table width="100%">
<tr><td width="20%">&nbsp;</td>
...
<tr><td width="20%">&nbsp;</td>
<td width="30%" align="right">Utilis&eacute;e pour la Moyenne?&nbsp;&nbsp;&nbsp;<br><br></td> 
<td width="50%"><input type="checkbox" name="Moyenne" id="Moyenne" value="1"></td></tr>

<tr><td width="20%">&nbsp;
<td width="30%"><div class="postcontent">
  <span class="readmore_b"><p>
    <a class="readmore" href="javascript:void(1);" onClick="CheckDropSubmitform('Temp_Name','id_sonde','AddTemp');">Ajouter</a></p></span>
  <div class="clear"></div>
</div>
</td>
<td width="50%" align="left"></td></tr>
